...
struct CurrentWeekHistorySection: View {
    ...
    var body: some View {
        Section {
            // The entry point to everything that spans shifts, at the head of
            // history rather than buried inside one shift: a summary of a day, a
            // week, a month or a chosen range is not a property of any single
            // shift in it.
            //
            ...
            // Beside the summaries rather than inside a shift: this one spans
            // every shift there is. Absent when history is empty, because an
            // export control over no records is an offer the app would have to
            // refuse.
            if hasAnyHistory {
    ...
    /// Which days the section is showing, and what it is not showing. An empty
    /// current week is never left looking like an empty app.
    @ViewBuilder
    private var footer: some View {
        if let week {
            let dates = week.rangeStatement(calendar: calendar, locale: locale)
            if shifts.isEmpty, otherShiftCount == 0 {
                Text("Completed shifts will appear here.")
            } else if shifts.isEmpty, hasOtherWeeks {
