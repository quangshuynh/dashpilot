...
| `DerivedData/`, `*.xcresult/` | Build output and test result bundles, written into the working tree by command-line runs |

`xcshareddata/xcschemes` is the exception next to `xcuserdata/` and **is** tracked: the `DashPilot`
and `DashPilotWidgets` schemes are what CI and the documented build commands name, so they belong to
the project rather than to whoever last opened it. See [Building](building.md#schemes).
