...
| Font bundle invariants | What a font change must not disturb, read off the bundle the app launches with: only Manrope bundled, no other family, every `UIAppFonts` entry ending in `.ttf` rather than `.tff`, the Open Font License bundled beside the fonts, the widget extension still embedded and carrying no font, and `NSSupportsLiveActivities`, the location background mode and the When In Use usage description still declared with no Always key |
